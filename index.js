...
const pluginName = 'gulp-avif-webp-lazyload'
		...
		try {
					...
					let regexpItem,
						...
						for (let i = 0; i < newUrlArr.length; i++) {
							if (newUrlArr[i].includes('.svg') || newUrlArr[i].includes('.gif')) {
								...
								for (k of extensions) {
									k = new RegExp(k, 'g')
									newUrlArr[i] = newUrlArr[i].replace(k, '.avif')
			...
			function pictureRender(url, imgTag) {
				if (imgTag.indexOf('data-src') > 0) {
					...
					return (`<picture><source data-srcset="${url}" srcset="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mNkYAAAAAYAAjCB0C8AAAAASUVORK5CYII=" type="image/avif"><source data-srcset="${url}" srcset="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mNkYAAAAAYAAjCB0C8AAAAASUVORK5CYII=" type="image/webp">${imgTag}</picture>`)
				} else {
					return (`<picture><source srcset="${url}" type="image/avif"><source srcset="${url}" type="image/webp">${imgTag}</picture>`)
				}
			}
